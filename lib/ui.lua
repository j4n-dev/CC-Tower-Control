...
local function metricValueStr(m)
    ...
    if m.type == MT.BAR then
        ...
        return formatNum(m.value) .. "/" .. formatNum(m.max) .. " " .. (m.unit or "") .. " " .. pct .. "%" ..
                   warnSuffix(m)
...
local function drawMetric(y, m, sourceLabel)
    ...
    put(W - #val, y, val, (warnSuffix(m) ~= "") and C.warn or C.dim, C.bg)
    y = y + 1
    ...
    if m.type == MT.BAR and m.max and m.max > 0 then
        ...
        y = y + 1
    elseif m.type == MT.RATE then
        local col = m.direction == "in" and C.rateIn or m.direction == "out" and C.rateOut or C.dim
...
local function drawAreaScreen(y, area, server, state, idx)
    ...
    for _, nodeId in ipairs(area.nodes or {}) do
        ...
        if nodeDef then
            ...
            addButton(1, y, W, y, function()
                return "node:" .. capturedId
            end)
...
local function drawDetailScreen(y, nodeId, server, state, idx)
    ...
    addButton(1, y, #backLabel + 2, y, function()
        return "back"
    end)
    ...
    if #controls > 0 then
        ...
        for _, control in ipairs(controls) do
            put(2, y, control.label or control.id, disabled and C.sliderDis or C.text, C.bg)
            ...
            if control.type == "toggle" then
                ...
                if not disabled then
                    ...
                    addButton(sx, y, W, y, function()
                        ...
                        if not ok then
                            print("[ui] Control error: " .. tostring(err))
                        end
                        return "refresh"
                    end)
                end
...
local function drawMEScreen(y, server)
    local ns = server.getNodeState("me_network") or {
        metrics = {},
        status = "offline"
    }
    ...
    if #watchItems > 0 then
        ...
        y = y + 1
        table.sort(watchItems, function(a, b)
...
local function render(server, areas, activeArea, activeNode, scrollY)
    ...
    for _, area in ipairs(areas) do
        ...
        put(tx, 1, label, isActive and C.tabText or C.tabDim, isActive and C.tabActive or C.tabBg)
        local capturedId = area.id
        addButton(tx, 1, tx + #label - 1, 1, function()
            return "tab:" .. capturedId
        end)
        tx = tx + #label + 1
    end
    ...
    local function vy(y)
        return y - scrollY + contentStart - 1
    end
    ...
    local function sdivider(y, label)
        local sy = vy(y)
        if sy < contentStart or sy > H then
            return
        end
        divider(sy, label)
    end
    local function sdrawMetric(y, m, sourceLabel)
        ...
        if vy(y + lines - 1) < contentStart then
            return y + lines
        end
        if vy(y) > H then
            return y + lines
        ...
        if sy >= contentStart and sy <= H then
            ...
            origPut(W - #val, sy, val, (warnSuffix(m) ~= "") and C.warn or C.dim, C.bg)
        end
    ...
    if activeArea == "me_network" then
        ...
        if #pinned > 0 then
            ...
            do
                local sy = vy(y);
                if sy >= contentStart and sy <= H then
                    sdivider(y, "Pinned from nodes")
                end
            end
            y = y + 1
            for _, p in ipairs(pinned) do
                y = sdrawMetric(y, p.metric, p.sourceLabel)
            end
        end
    ...
    elseif activeNode then
        ...
        saddButton(1, y, #backLabel, y, function()
            return "back"
        end)
        y = y + 2
        ...
        if #controls > 0 then
            ...
            for _, control in ipairs(controls) do
                do
                    local sy = vy(y);
                    if sy >= contentStart and sy <= H then
                        origPut(2, sy, control.label or control.id, disabled and C.sliderDis or C.text, C.bg)
    ...
    if scrollY > 0 then
        ...
        addButton(W, contentStart, W, contentStart, function()
            return "scroll:-3"
        end)
    end
    if totalVirtual > (H - contentStart + 1 + scrollY) then
        origPut(W, H, "\x19", C.dim, C.bg)
        addButton(W, H, W, H, function()
            return "scroll:3"
        end)
    end
end
