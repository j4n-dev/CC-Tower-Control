...
local function findOrCreateNode(nodeId, computerId, regMsg)
  ...
  print("[server] Auto-registered: " .. nodeId ..
        " (\"" .. nodeDef.label .. "\", area: " .. nodeDef.area .. ")")
...
local function pollME()
  ...
  for _, item in ipairs(registry.watchItems or {}) do
    local ok, data = pcall(function()
      return meBridge.getItem({ name = item.name })
    end)
    if ok and data then
      updateMetrics("me_network", {
        metrics.value(
          "item_" .. item.id,
          item.label,
          data.amount or 0,
...
local function sendControl(nodeId, capability, value)
  ...
  local response = protocol.sendAndWait(
    s.computerId,
    protocol.msgSet(capability, value),
    5
  )
